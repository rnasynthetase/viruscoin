...
uint256 hashGenesisBlock("0x82f677afef3f42961875dc12626880b541c4c59de5920738ee4280c1a16ae77a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Viruscoin: starting difficulty is 1 / 2^12
...
{
    // Viruscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 525600); // Viruscoin: 525600 blocks in ~3 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Viruscoin: 1 days
static const int64 nTargetSpacing = 3 * 60; // Viruscoin: 3 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Viruscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Viruscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Viruscoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Wed, 15 Jan 2014 04:35:58 GMT";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1389760558;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 387720059;

        if (fTestNet)
        {
            block.nTime    = 1389232713;
        ...
        assert(block.hashMerkleRoot == uint256("0x85c77a8d1c3fdf5de26f1d659281f1f21b96dfd38a8246740f2c43dc07e3b6ca"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Viruscoin: increase each by adding 2 to viruscoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ViruscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ViruscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ViruscoinMiner(CWallet *pwallet)
{
    printf("ViruscoinMiner started\n");
    ...
    try { loop {
        ...
        printf("Running ViruscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ViruscoinMiner terminated\n");
        throw;
    }
}

void GenerateViruscoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ViruscoinMiner, pwallet));
}
